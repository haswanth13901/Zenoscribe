...
| `LAST_SEEN_DEBOUNCE_SEC` | Optional | `60` | How long a process reuses its own `users.last_seen` write for a given user before writing again (`db.py`). Presence feeds only the admin console's online/offline indicator, so this trades second-exact accuracy for a large drop in write volume: at 60s an active user costs one row update a minute instead of one per request. `0` restores a write on every authenticated request. |
