...
describe("recorderReducer", () => {
  ...
  it("mic-denied goes to stopped with the exact original message format", () => {
    const state = recorderReducer(initialRecorderState, { type: "mic-denied", message: "Permission denied" });
  ...
  it("audio-fail stops with the exact original message format and clears partial", () => {
    let state = recorderReducer(initialRecorderState, { type: "partial", speaker: "user-1", text: "x" });
  ...
  it("server-error stops from listening with the exact original message format", () => {
    let state: RecorderState = { ...initialRecorderState, status: "listening", statusMessage: "listening" };
  ...
  it("ws-error stops with 'connection failed', regardless of prior status", () => {
    const state = recorderReducer({ ...initialRecorderState, status: "listening" }, { type: "ws-error" });
    expect(state.status).toBe("stopped");
  ...
  it("ws-close recovers to stopped from any active status (fixes the original's stuck-toggle gap while connecting)", () => {
    const fromConnecting = recorderReducer({ ...initialRecorderState, status: "connecting" }, { type: "ws-close" });
    expect(fromConnecting.status).toBe("stopped");
    expect(fromConnecting.isError).toBe(false);

    const fromListening = recorderReducer({ ...initialRecorderState, status: "listening" }, { type: "ws-close" });
  ...
  it("manual-stop stops an active session and is a no-op when idle", () => {
    const stopped = recorderReducer({ ...initialRecorderState, status: "listening" }, { type: "manual-stop" });
