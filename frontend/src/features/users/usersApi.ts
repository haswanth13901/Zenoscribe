...
import type { AdminUser, NewUserBody } from "./types";
...
export const usersApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    resetPassword: builder.mutation<{ ok: boolean }, { id: string; password: string }>({
      query: ({ id, password }) => ({ url: `/admin/users/${id}/password`, method: "POST", body: { password } }),
    ...
    setActive: builder.mutation<{ ok: boolean }, { id: string; is_active: boolean }>({
      query: ({ id, is_active }) => ({ url: `/admin/users/${id}/active`, method: "POST", body: { is_active } }),
      invalidatesTags: ["Users"],
    }),
