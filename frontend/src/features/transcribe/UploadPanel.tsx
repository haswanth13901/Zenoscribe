...
import { useGetLanguagesQuery } from "../languages/languagesApi";
import { extractApiError } from "../../lib/apiError";
import { useTranscribeTranslateMutation } from "./transcribeApi";
...
function formatUploadError(error: FetchBaseQueryError | SerializedError | undefined): string {
  ...
  return status ? `Transcription failed (${status}): ${message}` : `Transcription failed: ${message}`;
}
...
export function UploadPanel({ open, onClose }: UploadPanelProps): ReactElement | null {
  ...
  const { data: languagesData, isError: languagesError } = useGetLanguagesQuery(undefined, { skip: !open });
  ...
  return (
    <div className={styles.panel}>
      <div className={styles.card}>
        ...
        <input ref={fileInputRef} type="file" accept="audio/*" hidden onChange={onFileChange} data-testid="upload-file-input" />
        ...
        <div className={styles.row}>
          <div className={styles.langField}>
            ...
            <select id="upload-lang" value={targetLanguage} onChange={(e) => setTargetLanguage(e.target.value)}>
              {languages.map((l) => (
          ...
          <div>
            <button type="button" onClick={doTranscribe} disabled={!file || transcribeState.isLoading}>
              {transcribeState.isLoading ? "Transcribing..." : "Transcribe"}
            </button>
            <button type="button" onClick={doTranscribeAndTranslate} disabled={!file || translateState.isLoading}>
