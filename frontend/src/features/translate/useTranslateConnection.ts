...
import type { ServerMessage, TranslateSettings } from "./types";
...
function buildHello(token: string | null, settings: TranslateSettings) {
  const base = { token, speak: settings.speak, voice: settings.voice, diarize: settings.diarize, mode: settings.mode };
