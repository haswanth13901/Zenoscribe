...
import { initialTranslateState, translateReducer } from "./translateReducer";
...
describe("translateReducer", () => {
  ...
  it("mic-denied and audio-fail stop with the exact original message formats", () => {
    const micDenied = translateReducer(initialTranslateState, { type: "mic-denied", message: "Permission denied" });
    ...
    const audioFail = translateReducer({ ...initialTranslateState, status: "connecting" }, { type: "audio-fail", message: "boom" });
  ...
  describe("captions", () => {
    it("creates a new live utterance when none is in progress", () => {
      ...
      expect(state.utterances).toEqual([
        { speaker: "user-1", language: "es", spokenLang: null, source: "hola", translation: "hello", live: true, ts: 1000 },
      ]);
    });
    ...
    it("replaces (not appends) the live utterance's text on subsequent frames, keeping the original ts", () => {
      ...
      expect(state.utterances[0]).toMatchObject({ source: "hola mundo", translation: "hello world", ts: 1000 });
    });
  ...
  describe("audio-start", () => {
    ...
    it("is a no-op with no live utterance or no language", () => {
      const noUtterance = translateReducer(initialTranslateState, { type: "audio-start", language: "en" });
  ...
  it("server-error stops with the exact original message format", () => {
    const state = translateReducer({ ...initialTranslateState, status: "listening" }, { type: "server-error", message: "oops" });
  ...
  it("timeout stops with the server's message or the default", () => {
    const withMessage = translateReducer({ ...initialTranslateState, status: "listening" }, { type: "timeout", message: "custom" });
    expect(withMessage.statusMessage).toBe("custom");
    expect(withMessage.isError).toBe(false);

    const withoutMessage = translateReducer({ ...initialTranslateState, status: "listening" }, { type: "timeout" });
  ...
  it("ws-close recovers to stopped from any active status, and is a no-op when already idle/stopped", () => {
    const fromConnecting = translateReducer({ ...initialTranslateState, status: "connecting" }, { type: "ws-close" });
  ...
  it("ws-error stops with 'connection failed'", () => {
    const state = translateReducer({ ...initialTranslateState, status: "listening" }, { type: "ws-error" });
    expect(state.status).toBe("stopped");
  ...
  it("manual-stop stops an active session and is a no-op when idle", () => {
    const stopped = translateReducer({ ...initialTranslateState, status: "listening" }, { type: "manual-stop" });
  ...
  it("clear empties utterances independent of status", () => {
    const withData: TranslateState = {
      ...
      status: "stopped",
      utterances: [{ speaker: "user-1", language: "es", spokenLang: null, source: "a", translation: "b", live: false, ts: 1 }],
    };
