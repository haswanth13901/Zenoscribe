...
import { useElapsedTimer } from "../../features/translate/useElapsedTimer";
import { useTranslateConnection } from "../../features/translate/useTranslateConnection";
import type { TranslateMode, TranslateSettings } from "../../features/translate/types";
  ...
  return (
    <AppLayout user={user}>
      <div className={styles.toolbar}>
        ...
        >
          {state.status === "listening" && elapsed ? `${state.statusMessage} · ${elapsed}` : state.statusMessage}
        </span>
      </div>
      ...
      <div className={styles.controls}>
        <div className={styles.row1}>
          ...
          {mode === "one_way" && (
            <div className={styles.field}>
              ...
              <select id="target-language" value={targetLanguage} disabled={controlsLocked} onChange={(e) => setTargetLanguage(e.target.value)}>
                {languages.map((l) => (
              ...
              <div className={styles.hint}>Speak a different language than this. The translation appears on the right.</div>
            </div>
          )}
          ...
          {mode === "two_way" && (
            <>
              <div className={styles.field}>
                ...
                <select id="language-a" value={languageA} disabled={controlsLocked} onChange={(e) => setLanguageA(e.target.value)}>
                  {languages.map((l) => (
                    <option key={l.code} value={l.code}>
                      {l.name}
                    </option>
              ...
              <div className={styles.field}>
                ...
                <select id="language-b" value={languageB} disabled={controlsLocked} onChange={(e) => setLanguageB(e.target.value)}>
                  {languages.map((l) => (
                    <option key={l.code} value={l.code}>
                      {l.name}
                    </option>
          ...
          <div className={styles.field}>
            ...
            <select id="translate-voice" value={effectiveVoice} disabled={controlsLocked} onChange={(e) => setVoice(e.target.value)}>
              {voices.map((v) => (
        ...
        <div className={styles.row2}>
          <label className={styles.toggleField}>
            <input type="checkbox" checked={speak} disabled={controlsLocked} onChange={(e) => setSpeak(e.target.checked)} />
            Speak translation
          </label>
          <label className={styles.toggleField} title="Best in two-way mode with clear turn-taking and separate mics">
            <input type="checkbox" checked={diarize} onChange={(e) => setDiarize(e.target.checked)} />
            Label speakers
          </label>

          <div className={styles.viewSwitch}>
            <button type="button" data-testid="view-columns" className={view === "columns" ? styles.on : undefined} onClick={() => changeView("columns")}>
              Columns
            </button>
            <button type="button" data-testid="view-stacked" className={view === "stacked" ? styles.on : undefined} onClick={() => changeView("stacked")}>
              Stacked
            </button>
            <button type="button" data-testid="view-focus" className={view === "focus" ? styles.on : undefined} onClick={() => changeView("focus")}>
              Focus
            </button>
          </div>
