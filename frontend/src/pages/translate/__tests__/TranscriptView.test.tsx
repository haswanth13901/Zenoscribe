...
const langName = (code: string | null) => (code === "es" ? "Spanish" : code === "en" ? "English" : (code ?? ""));
...
describe("TranscriptView", () => {
  ...
  describe("stacked view", () => {
    it("shows a card per utterance with speaker name, language, source and translation together", () => {
      render(<TranscriptView utterances={[utt({ speaker: "user-1" })]} view="stacked" langName={langName} />);
    ...
    it("shows a spoken-language tag when spokenLang differs from the utterance's own language", () => {
      render(<TranscriptView utterances={[utt({ language: "es", spokenLang: "en" })]} view="stacked" langName={langName} />);
