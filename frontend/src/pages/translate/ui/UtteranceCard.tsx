...
import { fmtTime } from "@/shared/lib/formatters";
import type { Utterance } from "@/features/translate/model/types";
import { SpeakerBadge } from "@/pages/translate/ui/SpeakerBadge";
...
export function UtteranceCard({
  ...
  langName,
  large,
}: UtteranceCardProps): ReactElement {
  return (
    <div
      className={`${styles.card} ${utterance.live ? styles.partial : ""} ${large ? styles.large : ""}`}
    >
      ...
      <div className={styles.translation}>
        ...
        {utterance.spokenLang && (
          <span className={styles.spokenTag}>spoken in {langName(utterance.spokenLang)}</span>
        )}
      </div>
    </div>
  );
