...
import { PresenceDot } from "@/pages/admin/ui/PresenceDot";
...
function UserRow({ user, onMessage }: UserRowProps): ReactElement {
  ...
  async function handleDelete() {
    if (
      !window.confirm(
        `Delete ${user.username} and their ${user.recording_count} recording(s)? This cannot be undone.`,
      )
    )
      return;
    onMessage(null);
  ...
  return (
    <tr>
      ...
      <td>
        <span className={`${styles.tag} ${user.role === "admin" ? styles.tagAdmin : ""}`}>
          {user.role}
        </span>
      </td>
      <td>
...
export function UsersTable(): ReactElement {
  ...
  return (
    <div className={styles.panel}>
      ...
      {actionMessage && (
        <div className={actionMessage.ok ? styles.actionOk : styles.actionError}>
          {actionMessage.text}
        </div>
      )}
    </div>
