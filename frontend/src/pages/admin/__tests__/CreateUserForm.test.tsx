...
describe("CreateUserForm", () => {
  ...
  it("shows the server's 409 detail inline on a duplicate username", async () => {
    server.use(http.post("/api/admin/users", () => HttpResponse.json({ detail: "Username already taken" }, { status: 409 })));
    renderForm();
    await fillForm();
    await userEvent.click(screen.getByRole("button", { name: "Create user" }));
  ...
  it("shows the server's 400 detail inline for a short password", async () => {
    server.use(
      http.post("/api/admin/users", () =>
        HttpResponse.json({ detail: "Username required and password must be at least 8 characters" }, { status: 400 }),
      ),
    );
    renderForm();
    await fillForm();
