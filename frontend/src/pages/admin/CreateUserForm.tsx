...
export function CreateUserForm(): ReactElement {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setMessage({ text: extractApiError(err as FetchBaseQueryError | SerializedError).message, ok: false });
    }
  }
    ...
    <div className={styles.panel}>
      ...
      <div className={styles.grid}>
        ...
        <div>
          ...
          <input id="new-user-fullname" value={fullName} onChange={(e) => setFullName(e.target.value)} />
        </div>
        <div>
        ...
        <div>
          ...
          <select id="new-user-role" value={role} onChange={(e) => setRole(e.target.value as "user" | "admin")}>
      ...
      <div className={styles.actions}>
        <button type="button" className={styles.primary} onClick={handleCreate} disabled={isLoading}>
          Create user
        </button>
      </div>
      {message && <div className={`${styles.msg} ${message.ok ? styles.msgOk : styles.msgErr}`}>{message.text}</div>}
    </div>
  );
}
