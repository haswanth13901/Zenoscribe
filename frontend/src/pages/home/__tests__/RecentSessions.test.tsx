...
describe("RecentSessions", () => {
  it("shows a loading message while loading", () => {
    render(<RecentSessions recordings={undefined} isLoading isError={false} onRetry={vi.fn()} now={now} />);
    expect(screen.getByText("Loading...")).toBeInTheDocument();
  });

  it("shows a distinct error message with a working retry button", async () => {
    const onRetry = vi.fn();
    render(<RecentSessions recordings={undefined} isLoading={false} isError onRetry={onRetry} now={now} />);
  ...
  it("shows the empty-state message when there are no recordings", () => {
    render(<RecentSessions recordings={[]} isLoading={false} isError={false} onRetry={vi.fn()} now={now} />);
  ...
  it("renders at most 4 rows even with more recordings", () => {
    ...
    render(<RecentSessions recordings={recordings} isLoading={false} isError={false} onRetry={vi.fn()} now={now} />);
